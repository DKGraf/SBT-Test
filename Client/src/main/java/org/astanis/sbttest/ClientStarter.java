...
public class ClientStarter {
	public static void main(String[] args) {
		...
		for (int i = 0; i < 10; i++) {
			new Thread(new Caller(clientImpl)).start();
		}
	}
	...
	private static class Caller implements Runnable {
		...
		public void run() {
			while (true) {
				...
				clientImpl.remoteCall("service1", "sleep", new Object[]{1000L});
				logger.info("Current Date is: " + clientImpl.remoteCall("service1", "getCurrentDate", new Object[]{}));
				Integer x = new Random().nextInt(1000);
				Integer y = new Random().nextInt(1000);
				logger.info(x.toString() + " multiply by " + y.toString() + " equals " +
					clientImpl.remoteCall("service2", "multiply", new Object[] {x, y}));
			}
		}
	}
}
